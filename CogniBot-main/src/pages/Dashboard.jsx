...
//         <h1 className="mb-8 text-3xl font-extrabold text-slate-900">My Bookings Dashboard</h1>
...
//                     key={booking.id} 
//                     className="flex flex-col gap-4 p-6 transition-all border cursor-default rounded-2xl border-slate-200 hover:border-green-300 hover:shadow-md hover:bg-slate-50"
//                   >
...
//                         </div>
//                         <div>
//                           <p className="text-lg font-bold text-slate-900">Station {booking.stationId.substring(0, 6)}...</p>
...
//                         <div className="flex items-baseline justify-end gap-1 mb-1 text-xl font-black text-slate-900">
//                           <span className="text-base text-green-600">₹</span>{booking.amount}
//                         </div>
...
//                     <div className="flex items-center justify-between pt-4 text-sm font-medium border-t border-slate-100 text-slate-500">
...
//                 <div className="flex items-center justify-center w-20 h-20 mb-4 border-2 border-dashed rounded-full border-slate-300 bg-slate-50">
//                   <Activity size={32} />
//                 </div>
//                 <p className="text-lg font-bold text-slate-500">No recent bookings found.</p>
//                 <p className="mt-1 text-sm">Head to the Live Map to book an EV slot!</p>
//               </div>
//             )}
//           </motion.div>
//         </div>
...
const BookingCard = ({ booking, idx }) => {
  const st = statusStyle(booking.bookingStatus);
  const dateStr = booking.slotTime?.seconds
    ? new Date(booking.slotTime.seconds * 1000).toLocaleString('en-IN', {
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
          ...
          <div>
            <p style={{
              ...
              Station {booking.stationId?.substring(0, 8).toUpperCase()}
            </p>
            <p style={{
    ...
    <div style={{ minHeight: '100vh', background: '#fff', paddingTop: 96, paddingBottom: 80, position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 24px', position: 'relative', zIndex: 10 }}>
        ...
        <div style={{
          ...
          <StatCard icon={<Activity />} label="Total Bookings" value={bookings.length} delay={0.1} />
          <StatCard icon={<Zap />}      label="Total Spent"    value={`₹${totalSpent}`} accent delay={0.18} />
          <StatCard icon={<Radio />}    label="Active Slots"   value={bookings.filter(b => b.bookingStatus?.toLowerCase() === 'active').length} delay={0.26} />
          <StatCard icon={<Cpu />}      label="Completed"      value={bookings.filter(b => b.bookingStatus?.toLowerCase() === 'completed').length} delay={0.34} />
        </div>
